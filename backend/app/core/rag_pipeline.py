...
İki ana akış:
1. Ingestion Pipeline: Dosya yüklendiğinde → metin çıkar → temizle → parçala → embedding → indeksle
2. Query Pipeline: Soru sorulduğunda → cache kontrol → ara → rerank → ebeveyn getir → LLM

PRD Referans: Bölüm 4.2 ve 4.3 (Akış diyagramları)
"""
...
class RAGPipeline:
    ...
    Kullanım:
        pipeline = RAGPipeline(embedder, vector_store, reranker, llm_client, cache)
    ...
    def __init__(
        ...
        Args:
            ...
            final_top_k: Reranking sonrası kaç chunk LLM'e gitsin (varsayılan: 5)
        """
    ...
    async def ingest_document(
        ...
        # Yeni doküman eklendi → ilgili cache'leri invalidate et (opsiyonel iyileştirme)
        # Şu an pasif — cache zaten TTL ile temizleniyor
    ...
    async def query(
        ...
        context_chunks = await self._retrieve_and_resolve(
    ...
    async def query_stream(
        ...
        kaynak bilgisi yield edilebilir (PRD Bölüm 16.3).
        """
        # Cache kontrolü
        if self.cache:
            cached = await self.cache.get(question)
